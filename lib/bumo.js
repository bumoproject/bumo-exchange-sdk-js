...
proto.sendBu = function* (options) {
  try {
    ...
    if (err.message === 'Assertion failed') {
      return this._util.responseError(errorDesc.INVALID_ARGUMENTS);
    }
...
BUMOSDK.initOptions = function initOptions(options) {
  ...
  if (!Array.isArray(opts.ips) || !opts.ips.length) {
    throw new Error('The type of `ips` must be an Array and It\'s length is greater than 0');
  }
  ...
  opts.ips.forEach(ip => {
    ...
    if (!ipArr[0] || ipArr[0].trim().length === 0) {
      throw new Error('`ips` format is incorrect.');
    }

    if (!ipArr[1] || is.NaN(parseInt(ipArr[1]))) {
      throw new Error('`ips` format is incorrect.');
    }
    ips.push(`${ipArr[0].trim()}:${parseInt(ipArr[1])}`);
  });
