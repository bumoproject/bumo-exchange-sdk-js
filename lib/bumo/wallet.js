...
proto.getTransactionHistory = function* (options = {}) {
  ...
  if (res.error_code !== 0) {
    return this._util.sendError(errorDesc.FAIL);
  }

  return this._util.sendData(res.result);
};
...
proto.sendBu = function* (params) {
  try {
    ...
    if (!keypair.checkEncPrivateKey(from)) {
      return this._util.sendError(errorDesc.INVALID_PRIVATE_KEY);
    }

    if (!keypair.checkAddress(to)) {
      return this._util.sendError(errorDesc.INVALID_ADRESS);
    }
    ...
    return this._util.sendData(result);
  } catch (error) {
    // console.log(error);
